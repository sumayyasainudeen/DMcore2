...
def ex_task_form(request,taskid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    
    w = Work.objects.get(id=taskid)

    requird_field= Required_Field.objects.filter(work_id=w,required_task_name=w.task)
    work_id=Work.objects.get(id=taskid)
    try:
        lead_file = Lead.objects.get(file_work_id=work_id)   
    except Lead.DoesNotExist:
        lead_file=None

    lead_files = Lead.objects.filter(file_work_id=work_id)
    data = []
    ...
    for lead_file in lead_files:
        ...
        df = pd.read_excel(lead_file.file.path)
        num_data_rows = len(df) 
        
        # Convert the DataFrame to a list of dictionaries for easy rendering in the template
        df_data = df.to_dict('records')
        data.append({'file_name': lead_file.file_name, 'data': df_data})
       

    #saveing the file data count to lead table
    for lead_file in lead_files:
        lead_file.lead_count= num_data_rows
        lead_file.save()


    # save the achived data count to work table
   
    work_id.achived=num_data_rows
    work_id.save()

    context={
        "usr":usr,
        "requird_field":requird_field,
        'work_id':work_id,
        'lead_file':lead_file,
        'total_lead_files': data,
        'num_data_rows':num_data_rows
       
    }
    return render(request, 'executive/ex_daily_work_form.html',context)


def ex_create_excel_file(request,work_id):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)

    requird_field= Required_Field.objects.filter(work_id=Work.objects.get(id=work_id))
    work_id=Work.objects.get(id=work_id)
    lead_file=None
    
    if request.method == 'POST':

        file_name = f"{ request.POST['file_name']}.xlsx"  # Set your desired file name here
        full_file_path = os.path.join('media', 'excel_files', file_name)

        wb = Workbook()
        ws = wb.active

        # Get the column headers from the 'RequiredField' model and write them to the Excel file
        headers = list(Required_Field.objects.values_list('requiredfield_name', flat=True).filter(work_id=work_id,required_task_name=work_id.task))
       
        ws.append(headers)

        # Add your data to the Excel file
        # For example, you can get data from another model and write it to the file
        # data = YourOtherModel.objects.values_list('field1', 'field2', ...)
        ...
        # Save the file in the 'Lead' model
        lead_file = Lead.objects.create(file_name=file_name)
        lead_file.file.save(file_name, open(full_file_path, 'rb'))
        lead_file.file_work_id=work_id
        lead_file.exe_id = usr
        lead_file.save()
...
def ex_file_save(request,file_wid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)

    work_id=Work.objects.get(id=file_wid)
    requird_field= Required_Field.objects.filter(work_id=Work.objects.get(id=file_wid),required_task_name=work_id.task)
   
    if request.method == 'POST':
        fid = request.POST['file_id']
        lead_file = Lead.objects.get(id=fid)
        lead_file.file = request.FILES.get('lead__file')
        lead_file.save()
        print('file updated.')
    ...
    context={
        ...
        'work_id':work_id,
        'lead_file':lead_file,
        'total_lead_files': data,'num_data_rows':num_data_rows
    }
...
def ex_task_lead_form(request):
    ...
    if request.method == 'POST':
        requird_field= Required_Field.objects.filter(work_id=Work.objects.get(id=int(request.POST['wid'])))
        work_id=Work.objects.get(id=int(request.POST['wid']))
        l1=[]
        for key in request.POST:
            if key != 'csrfmiddlewaretoken':
                print(key)
                l1.append((request.POST[key]).capitalize())   
        print(l1) 
        for i in  requird_field:  
            a=request.FILES.get('i.requiredfield_name')  
            l1.append(a)
        print(l1) 
    context={
         "usr":usr,
...
def ex_weekly_rep_clint_det(request,id):
    ...
    context={
        ...
        "work_as":work_as,
        "rep":rep,
        "work_tb":work_tb,'id':id
    }
    return render(request, 'executive/ex_weekly_rep_det.html',context)


def exe_client_report_view(request,clid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work_as=work_asign.objects.filter(exe_name=ids,client_name_id=clid)

    work_details = Smo_socialmedia.objects.filter(smo_user_id=ids,smo_client_name_id=clid)
   
    rep=progress_report.objects.filter(user=usr,work__in=work_as.values_list('work',flat=True))
    context={
        "usr":usr,
        "work_as":work_as,
        "rep":rep,
    ...
    return render(request, 'executive/ex_client_rep.html',context)
    
def exe_client_perfomance_view(request,perf_clid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work_as=work_asign.objects.filter(exe_name=ids,client_name_id=perf_clid)

    work_details = Smo_socialmedia.objects.filter(smo_user_id=ids,smo_client_name_id=perf_clid)
    rep=progress_report.objects.filter(user=usr,work__in=work_as.values_list('work',flat=True))
    ...
    #previous Week date pickng 
    current_week_start = current_date - timedelta(days=current_date.weekday())

    prev_w_start_date = current_week_start - timedelta(days=7)
    prev_w_end_date = prev_w_start_date + timedelta(days=6)

    prev_w_start_date = prev_w_start_date.date()
    prev_w_end_date = prev_w_end_date.date()

    print('pstart_date:',prev_w_start_date)
    print('pend_date:',prev_w_end_date)

    # current Week date pickng 

    current_w_start_date=current_date - timedelta(days=current_date.weekday())
    current_w_end_date= current_w_start_date + timedelta(days=6)
    current_w_start_date = current_w_start_date.date()
    current_w_end_date =current_w_end_date.date()

    print('cstart_date:',current_w_start_date)
    print('cend_date:',current_w_end_date)

    for w in work_as:

        try:
            perf_task = perfomance.objects.get(client_work_id=w.id)

        except perfomance.DoesNotExist:
            perf_task = perfomance()

            perf_task.client_perf_name = w.client_name
            perf_task.client_work = w.work
            perf_task.pref_exe_name = w.exe_name
            perf_task.save()

        previous_work = Smo_socialmedia.objects.filter(smo_user_id=ids,smo_client_name_id=perf_clid,smo_work_id=w.id,
                                                       smo_start_date__gte=prev_w_start_date,smo_end_date__lte=prev_w_end_date)
        
       
        current_work = Smo_socialmedia.objects.filter(smo_user_id=ids,smo_client_name_id=perf_clid,smo_work_id=w.id,
                                                       smo_start_date__gte=current_w_start_date,smo_end_date__lte=current_w_end_date)
        
        if previous_work:

            prev_value = sum(p.smo_count for p in previous_work)
            current_value =sum(c.smo_count for c in current_work) 

            print('previ:',prev_value)
            print('couurn:',current_value)

            perf_task.previous_week = prev_value
            perf_task.current_week = current_value

            if prev_value < current_value:
                pref = prev_value
                pref = pref + ((prev_value/100)*35)
                print('couurn:',pref)
                if pref <= current_value:
                    
                    perf_task.week_perfomance = '100 %'
                    perf_task.previous_week = pref
                else:
                    
                    precent = (current_value/pref)*100
                    precent = round(precent,2)
                    perf_task.week_perfomance = f"{precent}%"
                    perf_task.previous_week = pref

            else:
                precent = (current_value/prev_value)*100
                precent = round(precent,2)
                perf_task.week_perfomance =  f"{precent}%"
                perf_task.previous_week = prev_value
        else:
           perf_task.week_perfomance = 'No Previous Work'
           perf_task.previous_week = 0
        
        perf_task.save()

    perf=perfomance.objects.filter(client_perf_name=perf_clid,pref_exe_name_id=ids)
    ...
    return render(request, 'executive/ex_client_perfomance.html',context)


def ex_weekly_rep_clint_det_smo(request,smo_wid):  #shebin created
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work_as=work_asign.objects.filter(work_id=smo_wid)
    work = Work.objects.get(id=smo_wid)
   
    work_tb=Work.objects.filter(id=smo_wid)
    rep=progress_report.objects.filter(user=usr)
    context={
        "usr":usr,
        "work_as":work_as,
    ...
    return render(request, 'executive/ex_wekly_workrep_det.html',context)


def ex_weekly_rep_clint_det_work_details(request,workd_wid):  #shebin created
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work_as=work_asign.objects.filter(work_id=workd_wid)
    work = Work.objects.get(id=workd_wid)
   
    work_tb=Work.objects.filter(id=workd_wid)
    rep=progress_report.objects.filter(user=usr)
    works_details = Smo_socialmedia.objects.filter(smo_work_id=work).order_by('-id')
    context={
        "usr":usr,
        "work_as":work_as,
        "work":work,
    ...
    return render(request, 'executive/ex_wekly_workrep_details.html',context)


def ex_weekly_rep_smo_socialmedia(request): #shebin 
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    ...
    if request.method == "POST":
        ...
        if 'Facebook' == request.POST.get("fb_select"):
            name = request.POST.get("fb_txt")
            number = request.POST.get("fb_num")
            file = request.FILES.get("fb_file")
            d1 = request.POST.get("fbst_dt")
            d2 = request.POST.get("fbed_dt")
            wid = request.POST.get("fb_workid")

            w=Work.objects.get(id=wid)

            smo_save = Smo_socialmedia()
            smo_save.smo_work = w
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("fb_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
            smo_save.smo_end_date = d2
            smo_save.save()

                
            return JsonResponse({"status": "success"})
        
        if 'Twitter' == request.POST.get("tw_select"):
            name = request.POST.get("tw_txt")
            number = request.POST.get("tw_num")
            file = request.FILES.get("tw_file")
            d1 = request.POST.get("twst_dt")
            d2 = request.POST.get("twed_dt")
            wid = request.POST.get("tw_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("tw_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Pinterest' == request.POST.get("pin_select"):
            name = request.POST.get("pin_txt")
            number = request.POST.get("pin_num")
            file = request.FILES.get("pin_file")
            d1 = request.POST.get("pinst_dt")
            d2 = request.POST.get("pined_dt")
            wid = request.POST.get("pin_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("pin_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Linkedin' == request.POST.get("link_select"):
            name = request.POST.get("link_txt")
            number = request.POST.get("link_num")
            file = request.FILES.get("link_file")
            d1 = request.POST.get("linkst_dt")
            d2 = request.POST.get("linked_dt")
            wid = request.POST.get("link_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("link_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Instagram' == request.POST.get("insta_select"):
            name = request.POST.get("insta_txt")
            number = request.POST.get("insta_num")
            file = request.FILES.get("insta_file")
            d1 = request.POST.get("instast_dt")
            d2 = request.POST.get("instaed_dt")
            wid = request.POST.get("insta_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("insta_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Tumblr' == request.POST.get("tumb_select"):
            name = request.POST.get("tumb_txt")
            number = request.POST.get("tumb_num")
            file = request.FILES.get("tumb_file")
            d1 = request.POST.get("tumbst_dt")
            d2 = request.POST.get("tumbed_dt")
            wid = request.POST.get("tumb_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("tumb_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Directories' == request.POST.get("diry_select"):
            name = request.POST.get("diry_txt")
            number = request.POST.get("diry_num")
            file = request.FILES.get("diry_file")
            d1 = request.POST.get("diryst_dt")
            d2 = request.POST.get("diryed_dt")
            wid = request.POST.get("diry_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("diry_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'YouTube' == request.POST.get("yt_select"):
            name = request.POST.get("yt_txt")
            number = request.POST.get("yt_num")
            file = request.FILES.get("yt_file")
            d1 = request.POST.get("ytst_dt")
            d2 = request.POST.get("yted_dt")
            wid = request.POST.get("yt_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("yt_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
        ...
        if 'Quora' == request.POST.get("qra_select"):
            name = request.POST.get("qra_txt")
            number = request.POST.get("qra_num")
            file = request.FILES.get("qra_file")
            d1 = request.POST.get("qrast_dt")
            d2 = request.POST.get("qraed_dt")
            wid = request.POST.get("qra_workid")
            ...
            smo_save.smo_user = usr
            smo_save.smo_client_name = w.client_name
            smo_save.smo_platform = request.POST.get("qra_select")
            smo_save.smo_platform_title = name
            smo_save.smo_count = number
            smo_save.smo_files = file
            smo_save.smo_start_date = d1
...
def ex_weekly_rep_details_add(request): #shebin 
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    
    if request.method == "POST":
    
        name1=request.POST.get("txt1")
        name2 = request.POST.get("txt2")
        number = request.POST.get("num")
        file = request.FILES.get("file")
        d1 = request.POST.get("st_dt")
        d2 = request.POST.get("ed_dt")
        wid = request.POST.get("workid")

        w=Work.objects.get(id=wid)

        details_save = Smo_socialmedia()
        details_save.smo_work = w
        details_save.smo_user = usr
        details_save.smo_client_name = w.client_name
        details_save.smo_platform = name1
        details_save.smo_platform_title = name2
        details_save.smo_count = number
        details_save.smo_files = file
        details_save.smo_start_date = d1
        details_save.smo_end_date = d2
        details_save.save()

        works_details = list(Smo_socialmedia.objects.filter(smo_work_id=wid).values().order_by('-id'))  # Convert queryset to list of dictionaries
        return JsonResponse({"status": "success","works_details":works_details})    
    
    works_details = list(Smo_socialmedia.objects.filter(smo_work_id=wid).values().order_by('-id'))
    return JsonResponse({"status": "error","works_details": works_details})

    
def ex_weekly_rep_smo_socialmedia_details(request,smo_wid):#shebin
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
   
    smo_works=Smo_socialmedia.objects.filter(smo_work_id=smo_wid)
    context={
        "usr":usr,
        "smo_works":smo_works,
    }
    return render(request, 'executive/ex_view_smo_work_clint.html',context)
...
def ex_view_clint_det(request,id):
    ...
    work=Work.objects.get(id=id)
    requiredField = Required_Field.objects.filter(work_id=work)
    context={
        "usr":usr, 
        "client":work,
...
def he_work_asign(request,pk):
    ...
    tasks = Client_Task.objects.filter(Client_Id=client.id)
    tls=user_registration.objects.filter(department='Team Lead')
    works = Work.objects.filter(client_name=client)
    # for w in works:
    #     tl = user_registration.objects.get(id=w.tl_id)
    #     w.tl_name = tl.fullname
    return render(request,'head/he_work_asign.html',{'client':client,'tls':tls,"usr":usr,"works":works,"tasks":tasks})

def he_update_task(request):
    print('update')
    ids=request.session['userid']
    if request.method=='POST':

        tsk_id=request.POST.get('tid')
        desc=request.POST.get('des')
        target=request.POST.get('target')
        file=request.POST.get('file')

        task = Client_Task.objects.get(id = tsk_id)
        task.Task_txt = desc
        task.Task_target = target
        task.Task_file = file
        task.save()
...
def he_progress_report(request,pk):
    ...
    try:
        ...
        prv_work=None
    return render(request,'head/he_progress_report.html',{'work':work,"usr":usr,"prv_work":prv_work,"sm":sm})
...
def he_work_add(request,id):
        ...
        usr = user_registration.objects.get(id=ids)
        if request.method == 'POST':
            cl_id = request.POST.get('client_id')
            tl_id = request.POST.get('tl')
            sdate = request.POST.get('d1')
            edate = request.POST.get('d2')

            tl = user_registration.objects.get(id = tl_id)

            task = Client_Task.objects.get(id = id)
            task.Tl_Id = tl
            task.Task_assign_status = 1
            task.Task_start_date = sdate
            task.Task_end_date = edate
            task.save()

            client = client_information.objects.get(id = cl_id)

            w=Work(task=task.Task_head,
                   client_task= task,
                   description=task.Task_txt,
                   start_date=sdate,
                   end_date=edate,
                   file_attached=task.Task_file,
                   cl_name=client.bs_name,
                   client_name=client,
...
def he_edit_work_assign(request,id):
        ids=request.session['userid']
        usr = user_registration.objects.get(id=ids)
        if request.method == 'POST':
            cl_id = request.POST.get('client_id')
            ...
            work = Work.objects.get(client_task= task)
            work.start_date = sdate
            work.end_date=edate
            work.tl_id=tl_id
            work.save()
...
#         client=client_information.objects.get(id=id)
#         w=Work(task=task,description=des,start_date=sdate,end_date=edate,file_attached=file,cl_name=client.bs_name,client_name=client,tl_id=tl)
#         w.save()
...
#         c=work_asign(work_id=w.id,tl_id=tl,client_name_id=client.id)
#         c.save()
#         return HttpResponse({"message": "success"})
...
def tc_view_current_leads(request):
    ...
    status_values=['no','Non calling','Will inform','Interested','Want to visit office', 'Please send details through WhatsApp','Join later','Call you back','Will contact after enquiry','Will contact if interested','Follow up']
    ...
    return render(request, 'telecaller/tc_view_current_leads.html', {'caller': caller,"usr":usr,'cur_leads':cur_leads,"count":count,"cdate":cdate})
...
def tc_update_status(request):
    ...
    assign_dt = lead.assign_dt
    tc_perf = TelecallerPerformance.objects.get(assign_date = assign_dt)

    assigned_cnt = tc_perf.assign_lead_count
    call_count = tc_perf.call_lead_count
    non_call_count = tc_perf.non_call_lead_count
    
    call_count = call_count+1
    non_call_count = non_call_count-1
    perf = round(call_count / assigned_cnt * 100, 2)

    tc_perf.call_lead_count = call_count
    tc_perf.non_call_lead_count = non_call_count
    tc_perf.performance_data = perf
    tc_perf.save()
...
def ad_delay_home(request):
    ...
    return render(request, 'admin/ad_delay_home.html', {'tls': tls,'usr':usr})
    
    
def ad_delay(request,tlid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exes=user_registration.objects.filter(department="Digital Marketing Executive",tl_id=tlid)
    if request.session.has_key('userid'):
        callerid = request.session['userid']
    else:
        return redirect('/')
    ...
    status_values = ["no"," "]
    work = Work.objects.filter(status__in=status_values,tl_id=tlid).order_by('-start_date')
    for i in work:
        if i.target is not None:
            i.difference = i.target - 10
        else:
...
def ad_delay_fltr(request,tlid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exes=user_registration.objects.filter(department="Digital Marketing Executive",tl_id=tlid)
    if request.session.has_key('userid'):
    ...
    if request.method=="POST":
        ...
        w_id = [obj.work_id for obj in wk_assign]
        status_values = ["no"," "]
        work = Work.objects.filter(id__in=w_id,start_date__gte=st_dt,end_date__lte=end_dt,status__in=status_values,tl_id=tlid).order_by('-start_date')
        print(work)
        for i in work:
            if i.target is not None:
                i.difference = i.target - 10
...
def he_executive_home(request,tlid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    executives = user_registration.objects.filter(department='Digital Marketing Executive',tl_id=tlid)
...
def he_view_exe_det(request,id):
    ...
    leads = Lead.objects.filter(exe_id_id=id)
...
def he_view_all_leads(request, date, id):
    ...
    lead_files = Lead.objects.filter(id=id)
    data = []
...
def he_save_create_work(request):
    ...
    if request.method == 'POST':
        ...
        for key in request.POST:
            if key.startswith('smo_field'):
                smo_field_create = Required_Field()
                smo_field_create.requiredfield_name = (request.POST[key]).capitalize()
                smo_field_create.client_id=client 
                smo_field_create.required_task_name=request.POST.get('smo',None)
                smo_field_create.save()

        # Determine how many dynamically added input fields you have based on their names
      
        for key in request.POST:
            if key.startswith('dynamic_field'):
                new_field_create = Required_Field()
                new_field_create.requiredfield_name = (request.POST[key]).capitalize()
                new_field_create.client_id=client
                new_field_create.required_task_name=request.POST.get('lc',None)
                new_field_create.save()
...
def tl_work_asign(request,pk):
    ...
    print(works)
    wa = work_asign.objects.filter(client_name=client)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=ids)
    return render(request,'team_lead/tl_work_asign.html',{'client':client,'exe':exe,"usr":usr,"works":works,"wa":wa})

def tl_work_add(request,id):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    if request.method == "POST":
        selected_executives = request.POST.getlist("ex")
        s_date = request.POST.get("d1")
        e_date = request.POST.get("d2")
        target = request.POST.get("target")
        print(selected_executives)
        print(s_date)
        print(e_date)
        wrk = Work.objects.get(id=id)
        print(wrk)
        clid = wrk.client_name_id
        for e in selected_executives:
            print(e)
            wa = work_asign(client_name_id = clid,work = wrk,exe_name_id = e,tl_id = wrk.tl_id,start_date = s_date,end_date = e_date,target = target)
            wa.save()
        wrk.assign_status = 1
        wrk.save()
...
def tl_change_progress_count(request):
    ...
    if request.method=="POST":
        ...
        s.smo_count_change_reason = reason
    ...
    if 'userid' in request.session:
        ...
        for e in exes:
            if e.tl_id == 0:
                ...
                print(e.tl_id)
                tl = user_registration.objects.get(id = e.tl_id)
                e.tl = tl.fullname
...
def he_workprogress_home(request):
    print('home')
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    return render(request,'head/he_workprogress_home.html',{"usr":usr,})

def he_workprogress_weekly_tl(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    print('hello')
    ...
    return render(request, 'head/he_workprogress_weekly_tl.html',context) 

def he_workprogress_executive_det(request,tlid):
    print("ha")
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=tlid)
    exe_ids = [exe.id for exe in exe]
    prgs=progress_report.objects.filter(user_id__in=exe_ids,status=0).order_by('-id')
    print(prgs)
    print("h")
    return render(request,'head/he_workprogress_executive_det.html',{'prgs':prgs,"usr":usr,})
...
def he_week_performance(request,wid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    print(wid)
    # work=Work.objects.get(id=wid)
    ...
    percentage_str = wp.week_perfomance
    percentage = float(percentage_str.rstrip('%'))
    return render(request,'head/he_week_performance.html',{"usr":usr,"wp":wp,"percentage":percentage})

def he_workprogress_monthly_tl(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    print('hello')
    ...
    return render(request, 'head/he_workprogress_monthly_tl.html',context) 
def he_workprogress_monthly_det(request,tlid):
    print("ha")
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=tlid)
    ...
    print(prgs)
    print("h")
    return render(request,'head/he_workprogress_monthly_det.html',{'prgs':prgs,"usr":usr,})

def he_progress_report_monthly(request,pk):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work=progress_report.objects.get(id=pk)
    sm = Smo_socialmedia.objects.filter(smo_work=work.work.id,smo_start_date=work.start_date,smo_end_date=work.end_date)
    try:
        ...
        prv_work=None
    return render(request,'head/he_progress_report_monthly.html',{'work':work,"usr":usr,"prv_work":prv_work,"sm":sm})

def he_all_leads_home(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    context={
    ...
    return render(request, 'head/he_all_leads_home.html',context) 

def he_executive_home_tls(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    context={
    ...
    return render(request, 'head/he_executive_home_tls.html',context)

#-----------------------------------------------------------------------------------------------sumayya---Telecaller-


def tc_upload_audio(request,lead_id):
    print('test')
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    if request.method == "POST":
        ...
        print('file')
        recording = CallRecordings(lead_id=lead_id,telecaller=usr,file=file,date=date.today())
        recording.save()
        return redirect('tc_view_current_leads')
    else:
        return redirect('tc_view_current_leads')

def tc_callrecordings_home(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    distinct_leads = CallRecordings.objects.filter(telecaller=usr).values('lead').distinct()
    lead_ids = [lead['lead'] for lead in distinct_leads]
    leads = All_leads.objects.filter(id__in=lead_ids)
    context={
        "usr":usr,
        "leads":leads

    }
    return render(request, 'telecaller/tc_callrecordings_home.html',context)

def tc_view_recordings(request,lid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    recordings = CallRecordings.objects.filter(lead_id=lid).order_by('-id')
    context={
        "usr":usr,
        "recordings":recordings

    }
    return render(request, 'telecaller/tc_view_recordings.html',context)

#-----------------------------------------------------------------------------------------------sumayya---admin-

def ad_workprogress_home(request):
    print('home')
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    return render(request,'admin/ad_workprogress_home.html',{"usr":usr,})

def ad_workprogress_weekly_tl(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    print('hello')
    ...
    return render(request, 'admin/ad_workprogress_weekly_tl.html',context) 

def ad_workprogress_executive_det(request,tlid):
    print("ha")
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=tlid)
    ...
    print(prgs)
    print("h")
    return render(request,'admin/ad_workprogress_executive_det.html',{'prgs':prgs,"usr":usr,})


def ad_progress_report(request,pk):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work=progress_report.objects.get(id=pk)
    sm = Smo_socialmedia.objects.filter(smo_work=work.work.id,smo_start_date=work.start_date,smo_end_date=work.end_date)
    try:
        ...
        prv_work=None
    return render(request,'admin/ad_progress_report.html',{'work':work,"usr":usr,"prv_work":prv_work,"sm":sm})


    
def ad_week_performance(request,wid):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    print(wid)
    # work=Work.objects.get(id=wid)
    ...
    percentage_str = wp.week_perfomance
    percentage = float(percentage_str.rstrip('%'))
    return render(request,'admin/ad_week_performance.html',{"usr":usr,"wp":wp,"percentage":percentage})

def ad_workprogress_monthly_tl(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    tls=user_registration.objects.filter(department="Team Lead")
    print('hello')
    ...
    return render(request, 'admin/ad_workprogress_monthly_tl.html',context) 
def ad_workprogress_monthly_det(request,tlid):
    print("ha")
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    exe=user_registration.objects.filter(department='Digital Marketing Executive',tl_id=tlid)
    ...
    print(prgs)
    print("h")
    return render(request,'admin/ad_workprogress_monthly_det.html',{'prgs':prgs,"usr":usr,})

def ad_progress_report_monthly(request,pk):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    work=progress_report.objects.get(id=pk)
    sm = Smo_socialmedia.objects.filter(smo_work=work.work.id,smo_start_date=work.start_date,smo_end_date=work.end_date)
    try:
        ...
        prv_work=None
    return render(request,'admin/ad_progress_report_monthly.html',{'work':work,"usr":usr,"prv_work":prv_work,"sm":sm})

#-------------------------------------------------------------------------------------------------Sumayya--telecaller

def tc_monthly_performance(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    context={
        "usr":usr,
    }
    return render(request, 'telecaller/tc_monthly_performance.html',context) 

from statistics import mean
def tc_monthly_performance_det(request):
    ids=request.session['userid']
    usr = user_registration.objects.get(id=ids)
    if request.method == "POST":
        s_date = request.POST['start_date']
        e_date = request.POST['end_date']
        performances = TelecallerPerformance.objects.filter(telecaller=usr,assign_date__gte=s_date,assign_date__lte=e_date)
        p_data = [p.performance_data for  p in performances]
        print(p_data)
        p_data_numeric = [float(data) for data in p_data]
        print(p_data_numeric)
        avg_p_data = sum(p_data_numeric) / len(p_data_numeric)
        
        context={
            'usr':usr,
            'performances':performances,
            'avg_p_data':avg_p_data,
        }
        return render(request, 'telecaller/tc_monthly_performance_det.html',context) 
